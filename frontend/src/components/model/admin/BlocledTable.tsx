...
const BlockedTable = () => {
  ...
  const handleEditRequest = (entityId: string, email: string, blocked: boolean, type: '一般ユーザー' | '店舗ユーザー') => {
  ...
  const handleUpdateEntityStatus = async () => {
    ...
    try {
      ...
      setBlockedEntities(prevEntities => prevEntities.map(entity => entity._id === editingEntityId ? data : entity));
